        ...
        ChannelInitializer<Channel> channelInitializer = new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel channel) throws Exception {
                try {
                    ...
                    logMessage(Level.SEVERE, "Failed to inject Channel " + channel + "!", e);
                }
            }
        };
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            logMessage(Level.SEVERE, "An error occurred while handling packet " + message.getClass().getSimpleName() + "!", e);
                            ReflectUtil.ReflectionResponse<String> response = ReflectUtil.getStringRepresentation(message, false);
                            if (response.hasResult()) {
                                logMessage(Level.SEVERE, "Packet dump: " + response.getValue(), null);
                            } else {
                                logMessage(Level.SEVERE, "Failed to retrieve packet dump!", response.getException());
                            }
                            result[0] = true;
                        }
                        result[1] = true;
